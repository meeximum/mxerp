...
public class TableKeyPopupPB extends PageBean implements Serializable {
	...
	public TableKeyPopupPB(ICustomizing newObject, ObjEntity objEntity) throws Exception {
	...
	private ROWDYNAMICCONTENTBinding generatePane() throws Exception {
		...
		for (ObjAttribute objAttribute : objEntity.getPrimaryKeys()) {
			...
			label.setText(Helper.getColumnNameForObject(objEntity.getDbEntityName(), objAttribute.getDbAttributeName()));
			rowNode.addSubNode(label);
			FIELDNode field = new FIELDNode();
			Reflect.on(newObject).call("set" + StringUtils.capitalize(objAttribute.getName()), StringUtils.EMPTY);
